...
def load_fonts(
    app_state: AppState,
):  # This is called by runnerParams.callbacks.LoadAdditionalFonts
    ...
    # Load the title font
    app_state.title_font = hello_imgui.load_font_ttf_with_font_awesome_icons(
        "fonts/Roboto/Roboto-BoldItalic.ttf", 18
    )

    # Load the emoji font
    font_loading_params_emoji = hello_imgui.FontLoadingParams()
    app_state.emoji_font = hello_imgui.load_font(
    ...
    app_state.large_icon_font = hello_imgui.load_font(
    ...
    app_state.color_font = hello_imgui.load_font(
...
def string_to_my_app_settings(s: str) -> MyAppSettings:
    ...
    try:
        ...
        hello_imgui.log(
            hello_imgui.LogLevel.error, f"Error while loading user settings: {e}"
        )
    return r
...
def demo_show_additional_window(app_state: AppState):
    ...
    if imgui.button("Show additional window"):
        ...
        additional_window.include_in_view_menu = (
            False  # this window is not shown in the view menu,
        )
        ...
        additional_window.gui_function = lambda: imgui.text(
            "This is the additional window"
        )
        hello_imgui.add_dockable_window(
            additional_window,
    ...
    imgui.set_item_tooltip(
        "By clicking this button, you can remove the additional window"
    )
...
Handling Japanese font is of course possible within ImGui / Hello ImGui!
            ...
            _, statics.poem = imgui.input_text_multiline(
                "##Poem", statics.poem, hello_imgui.em_to_vec2(15.0, 5.5)
            )

            # a popup with a modal window
            if imgui.button("Open Modal"):
                imgui.open_popup("MyModal")
            popup_opened, _ = imgui.begin_popup_modal(
                "MyModal", None, imgui.WindowFlags_.always_auto_resize
            )
            if popup_opened:
            ...
            if imgui.tree_node("Text Display"):
                ...
                imgui.text_wrapped(
                    "This is a long text that will be wrapped in the window"
                )
                imgui.tree_pop()
...
def demo_fonts(app_state: AppState):
    ...
    imgui.text_wrapped(
        "Mix icons "
        + icons_fontawesome_6.ICON_FA_FACE_SMILE
        + " and text "
        + icons_fontawesome_6.ICON_FA_ROCKET
    )
    if imgui.is_item_hovered():
...
def show_top_toolbar(app_state: AppState):
    ...
    if imgui.button(icons_fontawesome_6.ICON_FA_ADDRESS_BOOK):
        hello_imgui.log(
            hello_imgui.LogLevel.info, "Clicked on Address Book in the top toolbar"
        )
...
def show_right_toolbar(app_state: AppState):
    ...
    if imgui.button(icons_fontawesome_6.ICON_FA_CIRCLE_ARROW_LEFT):
        hello_imgui.log(
            hello_imgui.LogLevel.info, "Clicked on Circle left in the right toolbar"
        )
    if imgui.button(icons_fontawesome_6.ICON_FA_CIRCLE_ARROW_RIGHT):
        hello_imgui.log(
            hello_imgui.LogLevel.info, "Clicked on Circle right in the right toolbar"
        )
    imgui.pop_font()
...
def create_dockable_windows(app_state: AppState) -> List[hello_imgui.DockableWindow]:
    ...
    layout_customization_window.gui_function = lambda: gui_window_layout_customization(
        app_state
    )
    ...
    alternative_theme_window.gui_function = lambda: gui_window_alternative_theme(
        app_state
    )
...
def setup_my_theme():
    ...
    hello_imgui.apply_tweaked_theme(
        tweaked_theme
    )  # Note: you can also push/pop the theme in order to apply it only to a specific part of the Gui:  hello_imgui.push_tweaked_theme(tweaked_theme) / hello_imgui.pop_tweaked_theme()
    # Then apply further modifications to ImGui style
    imgui.get_style().item_spacing = ImVec2(
        6, 4
    )  # Reduce spacing between items ((8, 4) by default)
    imgui.get_style().set_color_(
        imgui.Col_.text, (0.8, 0.8, 0.85, 1.0)
    )  # Change text color
